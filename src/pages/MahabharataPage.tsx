...
import { mahabharataStories } from "@/data/stories";
...
export default function MahabharataPage() {
  ...
  return (
    <div className="min-h-screen" key={language}>
      ...
      >
        <img src={heroMahabharata} alt={t.nav.mahabharata} className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
        <div className="absolute inset-0 flex flex-col justify-end p-8">
          <div className="flex items-center gap-3 mb-2">
            <Book className="w-8 h-8 text-primary" />
          </div>
          <h1 className="font-display text-4xl md:text-5xl text-foreground mb-2">{t.nav.mahabharata}</h1>
        </div>
      </motion.section>

      <div className="grid gap-6">
        {mahabharataStories.map((story, index) => (
          <motion.div key={story.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="group">
            <Link to={`/mahabharata/${story.id}`}>
